...
void mx_message_to(t_widget_my *widge, const gchar *text) {
    ...
    widge->edit_icon = gtk_image_new_from_pixbuf(widge->edit_img);
    gtk_button_set_image (GTK_BUTTON(edit), widge->edit_icon);
    ...
    gtk_box_pack_start(GTK_BOX(box4), edit, 0, 0, 0);
    ...
    g_signal_connect(edit, "clicked", G_CALLBACK(print), widge->message_send[widge->message_id]);
