...
static void gtk_init_func(int argc, char *argv[], t_widget_my *widge) {
    ...
    gtk_css_provider_load_from_path (widge->theme, "client/src/themes/theme_1.css", NULL);
    gtk_style_context_add_provider_for_screen(gdk_screen_get_default(),
                                            GTK_STYLE_PROVIDER(widge->theme),
                                            GTK_STYLE_PROVIDER_PRIORITY_USER);

    widge->builder = gtk_builder_new ();
    if (gtk_builder_add_from_file (widge->builder, "client/src/login_new.glade", &error) == 0) {
